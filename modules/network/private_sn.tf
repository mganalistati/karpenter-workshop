...
resource "aws_subnet" "eks_private_sn_1a" {
  ...
  tags = merge({ Name = "${var.project_name}-subnet-private1-${data.aws_region.current.name}a" }, var.common_tags)
}
...
resource "aws_subnet" "eks_private_sn_2b" {
  ...
  tags = merge({ Name = "${var.project_name}-subnet-private2-${data.aws_region.current.name}b" }, var.common_tags)
}
...
resource "aws_route_table" "eks_private_rt_1a" {
  ...
  tags = merge({ Name = "${var.project_name}-rtb-private1-${data.aws_region.current.name}a" }, var.common_tags)
}
...
resource "aws_route_table" "eks_private_rt_2b" {
  ...
  tags = merge({ Name = "${var.project_name}-rtb-private2-${data.aws_region.current.name}b" }, var.common_tags)
}
